...
PRECISSION = 3.0
FRONT_HEIGHT = 300
MAX_NUMBER_HEIGHT_DIVIDER = 30.0

def detect_scaled(image, cascade, front_height, max_width):
    resized = cv2.resize(image, (front_height, front_height))
    rectsDetected = cascade.detectMultiScale(resized, scaleFactor=1.1, minNeighbors=1, minSize=(1,1), maxSize=(max_width,max_width))
...
for x in range(30, 50, 1): 
    ...
    for image_path in sorted(glob.glob('/home/kuba/Magisterka/local/img/test/number/number_detector/*')):
        ...
        rectsDetected = detect_scaled(image, cascade, FRONT_HEIGHT, int(float(FRONT_HEIGHT)/(float(x)*0.1)))
