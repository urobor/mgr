...
def detect_scaled(image, cascade, front_height=300, min_width=50,
        max_size=250, scale_factor=1.1, min_neighbors=1):
    (h,w,d) = image.shape
...
for x in range(105, 200, 5) :
    ...
    for image_path in sorted(glob.glob('/home/kuba/Magisterka/local/img/test/front/front_detector/*')):
        ...
        rectsDetected = detect_scaled(image, cascade, scale_factor=float(x)/100.0)
    ...
    print str(x) + " Correct: " + str(correct_hits) + "/" + str(all_occurences) + \
            " FAILS: " + str(wrong_hits) + "time: " + str(elapsed)
