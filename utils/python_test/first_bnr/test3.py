...
number_detector = cv2.CascadeClassifier('ocv/dev_number_2800_pos_2800_neg_n2_09995_pos.xml')
...
def detect_scaled(cascade, image, front_height, max_width, min_width):
    resized = cv2.resize(image, (front_height/2, front_height/3))
...
def detect(detector, image, height, mini, maxi, neighbors, scale, useHeight=True):
    (h,w) = image.shape
...
for img_path in sorted(glob.glob('/home/kuba/Magisterka/local/img/test/all/*')):
    ...
    if(len(fronts) > 0):
        ...
        # was f: 1820 n: 1609 r: 845
        numbers = detect_scaled(number_detector, upperleftcorner,
                285, 55, 45)
